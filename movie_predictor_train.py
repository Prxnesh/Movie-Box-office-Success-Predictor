# movie_predictor_train.py - IMPROVED VERSION
...
# NEW: Additional feature engineering
# 1. Budget efficiency
df_cleaned['budget_per_minute'] = df_cleaned['budget'] / df_cleaned['duration']

# 2. Log transformations (helps with skewed data)
df_cleaned['log_budget'] = np.log1p(df_cleaned['budget'])

# 3. Rating categories (helps model learn non-linear patterns)
...
# 5. Interaction features (often very powerful)
df_cleaned['rating_budget_interaction'] = df_cleaned['imdb_score'] * df_cleaned['log_budget']

# 6. Director/Actor popularity (frequency-based feature)
...
# 7. Calculate director/actor success rates (very powerful!)
...
df_cleaned['success_class'] = df_cleaned.apply(define_success, axis=1)
...
TOP_N_DIRECTORS = 100  # Increased from 50
TOP_N_ACTORS = 100     # Increased from 50
TOP_N_LANGUAGES = 15
...
# --- 6. Model Training with Multiple Improvements ---
